...
namespace semileptonic {
  ...
  void DrawCMSLabels(bool cmsprelim=true, double luminosity=0.0, double energy=7., double textSize=0.04)
  {
    ...
    if (cmsprelim)
      {
	label -> AddText(Form("CMS Preliminary, %2.1f fb^{-1} at #sqrt{s} = %2.0f TeV",luminosity/1000, energy));
      }
    else
      {
	label -> AddText(Form("CMS, %2.1f fb^{-1} at #sqrt{s} = %2.0f TeV",luminosity/1000, energy));
      }
