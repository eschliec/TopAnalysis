		    ...
		    const std::vector<pat::Muon>& muons, const double& weight=1.)
{
  ...
  if(muon!=muons.end()){
    ...
    for(int bin = 1; bin <= dRTrkN_->GetNbinsX(); ++bin){
      ...
      dREcalN_ ->Fill( dR, ecalDep ->depositAndCountWithin(dR).second  );
      dREcalPt_->Fill( dR, ecalDep ->depositAndCountWithin(dR).first );
      dRHcalN_ ->Fill( dR, hcalDep ->depositAndCountWithin(dR).second );
      dRHcalPt_->Fill( dR, hcalDep ->depositAndCountWithin(dR).first );
    }
    // fill summed deposits
    double stdDR = 0.3;
    double stdThreshold = 0.3;
    isoEcalN_->Fill( ecalDep->depositAndCountWithin(stdDR, reco::MuIsoDeposit::Vetos(), stdThreshold).second, weight );
    isoHcalN_->Fill( hcalDep ->depositAndCountWithin(stdDR, reco::MuIsoDeposit::Vetos(), stdThreshold).second, weight );
    isoTrkN_ ->Fill( trackDep->depositAndCountWithin(stdDR, reco::MuIsoDeposit::Vetos(), stdThreshold).second, weight );
